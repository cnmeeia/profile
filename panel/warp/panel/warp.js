        ...
        "Configs": {
            ...
            "i18n": {
                ...
                "zh-Hant": {
                    ...
                    "Data_Info_Used": "已用流量",
                    "Data_Info_Residual": "剩餘流量",
                    "Data_Info_Total": "總計流量",
                    "Data_Info_Unlimited": "無限流量"
                },
                "en": {
                    "IPv4": "Public IPv4",
                    "IPv6": "Public IPv6",
...
(async () => {
    const {
        ...
        Caches,
        Configs
    } = await setENV("Cloudflare", "1dot1dot1dot1", DataBase);
...
async function getENV(t, e, n) {
    ...
    function f(t, e, n) {
        e.split(".").reduce(((t, i, s) => t[i] = e.split(".").length === ++s ? n : t[i] || {}), t)
    }
}
...
async function setENV(name, platform, database) {
    ...
    let {
        ...
        Caches = {},
        Configs
    } = await getENV(name, platform, database);
    /***************** Prase *****************/
                    ...
                    if (error) throw new Error(error)
                        ...
                        switch (_data.success) {
                            case true:
                                ...
                                break;
                            case false:
                                if (Array.isArray(_data.errors)) _data.errors.forEach(error => {
                                    $.msg($.name, `code: ${error.code}`, `message: ${error.message}`);
                                })
                                break;
                            case undefined:
                                resolve(response);
